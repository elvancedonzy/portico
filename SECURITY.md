...
### 1. LAN-reachable web UI, no auth (planned fix: v0.2)
`server.py` binds `0.0.0.0:8765`. Anyone on your LAN can reach `POST /access_codes/create` via the add-on's HTTP proxy and create a lock code. **Mitigation until v0.2:** access the dashboard via HA Ingress only, block port 8765 at your router/firewall, or remove the `ports:` block from `portico/config.yaml` before install.

### 2. Booking.com email intake trusts unauthenticated email
...
### 4. No signed releases
Users installing from this GitHub repo pull whatever is on `main`. A compromised maintainer account = malicious code shipped. Planned for v0.2: signed tags and release notes.
...
- **Does not accept remote unlock commands.** The dashboard's `POST /locks/unlock_door` requires local network access; see issue #1 above.
...
We do not pin `seam` in v0.1 (planned for v0.2). If Seam ships a breaking change, `airbnb_extend.py` may crash on next run.
